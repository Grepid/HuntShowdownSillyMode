...
namespace AudioSystem
{
    public class AudioManager : MonoBehaviour
    {
        ...
        public Sound PauseMusic;
        private AudioPlayer pausePlayer;
        private AudioPlayer musicBeforePause;
        ...
        private void SpawnAudioSources()
        {
            ...
            DontDestroyOnLoad(masterSource);
        ...
        private static AudioPlayer GetValidSource()
        {
            ...
            foreach(AudioPlayer player in soundSources)
            {
                if (player == null) continue;
                if(player.AudioSource.isPlaying) continue;
                if(player.wasPausedByESC) continue;
                result = player;
                break;
            }*/
        ...
        /// <summary>
        /// Plays a sound that will attach to AudioManager to persist through level swaps. Only use for Music with 0 Spatial Blend and Maximum range 
        /// </summary>
        /// <param name="sound"></param>
        /// <returns></returns>
        public static AudioPlayer PlayPersistent(Sound sound)
        {
            if (!FullValidCheck) return null;
            AudioPlayer player;
            player = DefaultPlay(sound);
        ...
        /// <param name="sound"></param>
        /// <returns></returns>
        public static AudioPlayer PlayPersistent(string sound)
        {
            if (!FullValidCheck) return null;
            AudioPlayer player;
            player = DefaultPlay(sound);
        ...
        public static float SoundTypeVolume(SoundType type)
        {
            ...
            if (type == SoundType.Master) v = s_Volumes[SoundType.Master];
            return v;
        }

        public static float VolumeForSound(Sound sound)
        {
            if (!FullValidCheck) return 0;
            float v = sound.volume * SoundTypeVolume(sound.type);
            return v;
        }
        ...
        public static void AdjustAudioSource(AudioSource source, Sound sound)
        {
            ...
            source.volume = VolumeForSound(sound);
            ...
            //print($"adjusted {source.gameObject.name} audio source with the sound {sound.name}");
        }
        ...
        {
            if (!FullValidCheck) return;
            if (player.SoundClass == null) return;
        ...
        {
            if (!FullValidCheck) return;
            foreach (AudioPlayer player in AllActiveAudio)
            {
                if (!player.AudioSource.isPlaying) continue;
        ...
        public static void CrossFade(AudioPlayer In, AudioPlayer Out, float fadeTime)
        {
            ...
            else if (In == null && Out == null) return;
            FadeIn(In, fadeTime);
            FadeOut(Out, fadeTime);
        }
        ...
        private static IEnumerator FadeInEnum(AudioPlayer In, float fadeTime, bool fromCurrentVol)
        {
            ...
            while (!finished)
            {
                In.AudioSource.volume = Mathf.Lerp(startVol, VolumeForSound(In.SoundClass), (timeTaken / fadeTime));
